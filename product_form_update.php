...
<head>
 ...
 <?php
   ...
   $sql ="SELECT * FROM product_stock,brand_product,product_type,format_product,category WHERE product_stock.brand_id=brand_product.brand_id AND product_stock.type_id = product_type.type_id AND product_stock.format_id=format_product.format_id AND product_stock.id_category=category.id_category and product_code='$product_code'";
...
<div class="container" style="width: 100%;background-color: #66b3ff;border-bottom: 3%;border: 5px solid #000000">
  <div class="container" style="width:100%;background-color:#b3d9ff; border-radius: 2%;border-bottom: 3%;">
    ...
    <form class="form-horizontal"  action="product_update.php"   method="POST" enctype="multipart/form-data" id="form">
  ...
  <label class="control-label col-sm-2" for="typeproduct">ประเภทสินค้า :</label>
     ...
     <select  class="form-control col-md-1" id="id_category" name="id_category"  style="width:200px;" >
           <option  value="<?=$row_pro['id_category'];?>"><?=$row_pro['cateory_name'];?></option>
      <?php
  while($row = $result2->fetch_assoc())
  { ?>  
      <option value="<?=$row['id_category'];?>"  disabled="true"> <?=$row['cateory_name'];?> </option>  
 <?php } ?>  
     ...
     <label class="control-label col-sm-2">ชนิด:</label>
        <select class="form-control col-md-1"  id="type_id" name="type_id" style="width:200px;">
         <option  value="<?=$row_pro['type_id'];?>"><?=$row_pro['name_type'];?></option>
  

     </select>

  <label class="control-label col-sm-1">จำนวน:</label>
          <input type="text"   value="<?=$row_pro['qty'];?>" class="form-control col-sm-1" name="qty" id="qty"  style="width:100px;">
 ...
 <option  value="<?=$row_pro['brand_id'];?>"><?=$row_pro['brand_name'];?></option>
     ...
     <label class="control-label col-sm-1" >รูปแบบ:</label>
       ...
       <option  value="<?=$row_pro['format_id'];?>"><?=$row_pro['format_name'];?></option>
      
     </select>
      <label class="control-label col-sm-1">หน่วย:</label>
         <input type="text" class="form-control col-sm-1" value="<?=$row_pro['unit_name'];?>" id="unit_name" name="unit_name" style="width: 100px;" readonly>
    </div>
   </div>
...
<div class="form-group">
   <div class="col-sm-12">
      <label class="control-label col-sm-2">ชือสินค้า:</label>
        <input type="text" value="<?=$row_pro['name_product'];?>"  class="form-control col-sm-1" name="name_product" id="name_product" style="width:200px;">

        <label class="control-label col-sm-1" >ราคา :</label>
        <input type="text"  class="form-control col-md-2" value="<?=$row_pro['price'];?>"  id="price" name="price" style="width:100px;">
        <label class="control-label col-sm-1" >รูปสินค้า:</label>
         <input type="file"  class="form-control col-md-2"  id="img_product" name="img_product" style="width:200px;" value="<?=$row_pro['img_product'];?>" >  

<div class="form-group">
  <div class="col-sm-12">
     <label class="control-label col-sm-2"> โคดสินค้า:</label>
     <input type="text"  class="form-control col-md-2"  id="product_code" name="product_code" style="width:200px;" value="<?=$row_pro['product_code'];?>" readonly>
    </div>
  </div>
<div>
  
 <center><img src="upload/<?=$row_pro['img_product'];?>" height="200" width="200"></center>
</div>
<br>
<br>
      ...
      <div>
       ...
       <center><button type="submit" class="btn btn-primary" style="width:15%" >แก้ไขข้อมูล</button></center>
